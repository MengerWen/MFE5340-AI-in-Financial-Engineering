...
& "d:\MG\anaconda3\python.exe" scripts/build_integrated_notebook.py
```
...
The integrated teacher-facing notebook is generated by `scripts/build_integrated_notebook.py` and written to `Final_Project_Integrated_Showcase.ipynb`. It defaults to loading saved artifacts, tables, and figures; the full reproduction command cells only run after explicitly setting `RUN_FULL_PIPELINE = True` inside the notebook.
